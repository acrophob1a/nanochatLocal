...
运行方式：
  ...
  中文续训（从已有 SFT）：bash runs/chinese_sft.sh
"""
...
# 模型加载
parser.add_argument("--resume-from", type=str, default="base", choices=["base", "sft"],
                    help="checkpoint 来源：base=预训练；sft=已有 SFT（中文续训时用）")
parser.add_argument("--model-tag", type=str, default=None, help="要加载的模型 tag（如 d26）")
parser.add_argument("--model-step", type=int, default=None, help="要加载的模型 step，不指定则用最新")
# 训练步数
parser.add_argument("--num-iterations", type=int, default=-1,
                    help="DataLoader 出 batch 次数（非参数更新次数）；-1 表示按 epoch 结束。与 --optimizer-steps 二选一")
parser.add_argument("--optimizer-steps", type=int, default=None,
                    help="参数更新次数；指定后自动设 num_iterations = optimizer_steps × 梯度累积步数")
parser.add_argument("--num-epochs", type=int, default=0,
                    help="训练 epoch 数；>0 时跑完对应轮数后结束（配合 --chinese-only 反复刷中文）")
parser.add_argument("--chinese-only", action="store_true", help="仅使用 chinese_conversations.jsonl 训练")
parser.add_argument("--chinese-repeats", type=int, default=20,
                    help="--chinese-only 时把中文语料在混合池里重复几遍（75 条 × repeats 槽位）")
# 批大小
...
parser.add_argument("--init-lr-frac", type=float, default=1.0,
                    help="初始学习率为基础学习率的比例；从 sft 续训建议 0.1–0.3")
# 验证
...
# wandb：run=dummy、WANDB_MODE=disabled、或非主进程时不连 wandb
_wandb_mode = os.environ.get("WANDB_MODE", "").lower()
use_dummy_wandb = (
    args.run == "dummy"
    or not master_process
    or _wandb_mode in ("disabled", "offline", "dryrun")
)
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nanochat-sft", name=args.run, config=user_config)

# 从 base 或 chatsft checkpoint 加载模型与 tokenizer，phase=train 用于 SFT
print0(f"从 {args.resume_from}_checkpoints 加载模型 (tag={args.model_tag}, step={args.model_step})")
model, tokenizer, meta = load_model(
    args.resume_from, device, phase="train", model_tag=args.model_tag, step=args.model_step
)
# 从已有 SFT 续训时，保存文件名使用「全局步数」= 加载步数 + 本次训练步数（避免 model_000025 覆盖语义）
step_offset = 0
if args.resume_from == "sft":
    step_offset = args.model_step if args.model_step is not None else meta.get("step", 0)
    print0(f"续训：权重来自 step {step_offset}；本次运行 step 0 将保存为 model_{step_offset:06d}+N")
prior_batch_size = meta.get("device_batch_size") or meta.get("user_config", {}).get("device_batch_size")
if prior_batch_size is not None and args.device_batch_size > prior_batch_size:
    print0(f"警告：此前训练 device_batch_size 为 {prior_batch_size}，请确认 --device-batch-size 是否合适")
...
if args.optimizer_steps is not None:
    args.num_iterations = args.optimizer_steps * grad_accum_steps
    print0(f"--optimizer-steps {args.optimizer_steps} => num-iterations {args.num_iterations} (×{grad_accum_steps} 梯度累积)")
...
chinese_conversations_filepath = os.path.join(base_dir, "chinese_conversations.jsonl")
chinese_json = lambda: CustomJSON(filepath=chinese_conversations_filepath)
if args.chinese_only:
    repeats = max(1, args.chinese_repeats)
    train_dataset = TaskMixture([chinese_json() for _ in range(repeats)])
    val_dataset = CustomJSON(filepath=chinese_conversations_filepath, stop=15)
    print0(f"仅中文训练: {repeats}× chinese_conversations.jsonl => {len(train_dataset)} 槽位/epoch")
    if args.num_epochs > 0:
        print0(f"将训练 {args.num_epochs} 个 epoch（每条原文约见 {repeats} 次/epoch，共约 {repeats * args.num_epochs} 次/条）")
else:
    # 训练集：中文过采样 + 适量英文，避免 75 条中文被 460K 英文淹没
    train_dataset = TaskMixture([
        chinese_json(),
        chinese_json(),
        chinese_json(),
        chinese_json(),
        chinese_json(),  # 5x 过采样 chinese_conversations.jsonl
        CustomJSON(filepath=identity_conversations_filepath),
        SmolTalk(split="train", stop=20000),  # 子集英文对话，加快 epoch、减轻遗忘中文时的英文占比
    ])
    val_dataset = TaskMixture([
        CustomJSON(filepath=chinese_conversations_filepath, stop=15),
        SmolTalk(split="test", stop=2000),
    ])
...
def sft_data_generator_bos_bestfit(split, buffer_size=100):
    ...
    while True:
        ...
        if split == "train":
            current_epoch = epoch
            if args.num_epochs > 0:
                approx_progress = min((epoch - 1) / args.num_epochs, 1.0)
...
while True:
    ...
    if master_process and last_step and not args.dry_run:
        ...
        print0(f"保存 checkpoint：本次运行 {step} 步，全局 step {global_step} -> model_{global_step:06d}.pt")
